...
class Enemy(pg.sprite.Sprite):
    def __init__(self, enemy_type, waypoint):
        ...
        Enemy = db.select("SELECT * FROM ENEMY WHERE id = ?", (ENEMY_TYPES[enemy_type], ))
        ...
        #HP BAR
        self.curHPBar = pg.Rect(self.rect.left, self.rect.top - 50, self.rect.width, 50)
    ...
    def move(self):
        ...
        if (self.waypointIndex < len(self.waypoint)):
            ...
            self.distance = self.target_destination - self.pos  # d = (xB-xA,yB-yA)
        else:
            if self.attack:
                self.kill()
            self.attack = True
    ...
    def draw(self, surface):
        self.maxHPBar.update(self.rect.left + 10, self.rect.top + 17, self.rect.width - 20, 5)
        self.curHPBar.update(self.rect.left + 10, self.rect.top + 17, self.HP/self.maxHP * self.maxHPBar.width, 5)
