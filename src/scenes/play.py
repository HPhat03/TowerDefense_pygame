...
class Play(Scene):
    ...
    for i in range(3):
        for c in range(2):
            if count < 5:
                box = ItemBox((c + 1) * 2 * padding + c * width, (i+1)* padding + i*(width+2*padding), 100, "src/assets/MediumLevelBG.png", "NONE", "")
                teamTowers.append(box)
                count += 1
    sellingBT = Button(4*padding+width, 3*padding+2*(width+4*padding), width, 50, "SELL", 20, setting.button_color, "white")
    teamTowers.append(sellingBT)
    container = ControlsContainer(setting.WINDOW_WIDTH, 0, teamTowers, padding=padding, bgcolor=(113, 112, 113))

    UGPs = pg.sprite.Group()
    UGLb = Label(30, 10, 50, 50, "UPGRADE", color="white", size=20)
    UGPbx= PictureBox(UGLb.rect.left, UGLb.rect.top+UGLb.rect.height, 100, 100, "src/assets/MediumLevelBG.png")
    UGBT = Button(UGPbx.rect.width + UGPbx.rect.left + 10, UGLb.rect.top+UGLb.rect.height+40 , 150, 50, "upgrade", 20,
                  (255, 237, 41), "white")
