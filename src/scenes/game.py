...
class Game(Scene):
    ...
    PlayBt = Button(setting.WINDOW_WIDTH-120, setting.WINDOW_HEIGHT-60, 100, 50, "Play", 20, (112,187,68), "white")
    isEnabled = True
    ...
    padding = 5
    for c in range(0,5):
        box = ItemBox((c+1)*2*padding + c*100,2* padding, 100, "src/assets/MediumLevelBG.png", "NONE","")
        teamTowers.append(box)
    container = ControlsContainer(350,20,teamTowers,padding = padding, bgcolor="grey")
    ...
    nextBt = Button(Map_Loader.rect.left + Map_Loader.rect.width/2 + 200,
                    ...
                    50, 50, ">", 25, setting.button_color, "white")

    preBt = Button(Map_Loader.rect.left + Map_Loader.rect.width/2 - 200,
                    Map_Loader.rect.top + Map_Loader.rect.height + 10,
                    50, 50, "<", 25, setting.button_color, "white")
    MapNameLb = Label(preBt.rect.left+preBt.rect.width,nextBt.rect.top, nextBt.rect.left - (preBt.rect.left + preBt.rect.width), 50, text= "",bgcolor="white")
    controls = pg.sprite.Group()
    map_index = 0

    panel = pg.Rect(325, 10, container.rect.width + 50, nextBt.rect.top + nextBt.rect.height)
    panel_color = EzBt.bgcolor
    controls.add(Backpic,LvLabel,EzBt,MdBt,PlayBt,container, Map_Loader, nextBt, preBt, MapNameLb)

    @staticmethod
    def event_handler(event,login):
        if event.type == pg.KEYDOWN:
            if event.key == pg.K_s and login.isAuth:
                return Scenes.MENU
        return None
    ...
    def game(screen, login):
        ...
        pg.draw.rect(screen, color=Game.panel_color, rect=Game.panel, border_radius= 15)
        ...
        if Game.EzBt.isClicked():
            ...
            Game.panel_color = Game.EzBt.bgcolor
            Game.isEnabled = True
        if Game.MdBt.isClicked():
            Game.Background = Game.MdBg
            Game.panel_color = Game.MdBt.bgcolor
            Game.isEnabled = False
        if not Game.isEnabled:
            Game.PlayBt.bgcolor = "grey"
            Game.PlayBt.text = "COMMING SOON"

        else:
            Game.PlayBt.bgcolor = setting.button_color
            Game.PlayBt.text = "Play"
        Game.PlayBt.rect.width = Game.PlayBt.text_rect.width + 20
        Game.PlayBt.rect.left = setting.WINDOW_WIDTH - Game.PlayBt.rect.width - 20
