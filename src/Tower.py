...
class BattleTower(Tower):
    ...
    def isPlaceable(self, record):
        if record.map.map_data[self.tile_y * setting.MAP_WIDTH_TILE + self.tile_x] == record.map.path:
            return  False
        for i in record.towerGroup:
            if i.tile_x == self.tile_x and self.tile_y == i.tile_y:
                return False
...
class normalScout(BattleTower):
    ...
    def draw(self, surface):
        ...
        if self.isFocus:
            ...
            pg.draw.circle(self.boundSf, "white", (self.bound, self.bound), self.bound)
